...
LIT_DIR = ROOT / "3_literature_workflow" / "structured_summary"
SKILLS_LIBRARY = MEMORY_DIR / "skills_library.md"
BENCHMARK_CONFIG = ROOT / "1_config" / "base" / "benchmark_projects.json"
...
def _run_demo(rescuer: XiaoLongXiaRescuer) -> dict:
    ...
    data_path = ROOT / "6_experiment_execution" / "data" / "shandong_pmos_hourly.csv"
    if not data_path.exists():
        data_path = ROOT / "data" / "shandong_pmos_hourly.csv"

    df = power_common.read_numeric_timeseries(str(data_path)).tail(5000)
    metrics = power_models.train_eval_xgboost(df, optimized=True)

    out = ROOT / "6_experiment_execution" / "experiment_result" / "auto_demo_metrics.json"
...
def _run_full_baselines(rescuer: XiaoLongXiaRescuer) -> Path:
    ...
    log_path = ROOT / "6_experiment_execution" / "tuning_log" / "auto_full_pipeline_stdout.log"
...
def _load_round_metrics() -> dict:
    metrics = ROOT / "8_knowledge_asset" / "final_report" / "round_fixed_metrics.json"
    if not metrics.exists():
        metrics = ROOT / "report" / "round_fixed_metrics.json"
    if not metrics.exists():
        return {}
...
def execute_round(request_text: str, round_id: int, env: dict[str, str], rescuer: XiaoLongXiaRescuer, skip_full_baseline: bool) -> dict:
    ...
    summary_path = _write_text(
        ROOT / "3_literature_workflow" / "structured_summary" / f"agent_reading_round{round_id}.md",
    ...
    innovation_path = _write_text(
        ROOT / "4_research_hypothesis" / f"innovation_round{round_id}.md",
    ...
    review_path = _write_text(
        ROOT / "4_research_hypothesis" / "review_report" / f"innovation_review_round{round_id}.md",
